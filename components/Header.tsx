...
const Header = () => {
    ...
    return (
        <header className='z-[999] relative'>
        	<motion.div 
                className='fixed p-2 top-0 left-1/2 w-full h-auto flex rounded-none border-white border-opacity-40 bg-white bg-opacity-80 shadow-lg shadow-black/[.03] backdrop-blur-[.5rem] sm:top-6 sm:rounded-full sm:w-[36rem] '
                        ...
                        <span style={{
                            ...
                            transform:`
                                translate(
                                    ...
                                    , 0 )`
                        }} className= 'transition absolute bg-black/5 h-full left-0 rounded-full invisible sm:visible'/>
                    {
                        links.map((link,index)=>(
                            ...
                            >
                                <Link 
                                    className={`
                                        flex-1 flex-center text-center hover:text-gray-950
                                        transition rounded-full sm:bg-transparent 
                                         ${activeSection===link.name ?" text-gray-950  bg-black/5 ":" "} `}
                                    href={link.hash}
